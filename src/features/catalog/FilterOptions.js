...
function FilterOptions() {
  ...
  const isVisible = useSelector(selectShowFilterOptions);
    ...
    <React.Fragment>
      ...
      <div className={`${wrapper} ${isVisible ? show : null}`}>
        <div className={optionsGroup}>
          ...
          <button className={`${shiftBtn} ${filters.promotion ? shiftBtnActive : null}`} onClick={() => dispatch(setFilters({ filter: 'promotion', value: 'negation' }))} />
        </div>
