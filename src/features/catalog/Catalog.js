...
function Catalog() {
  ...
  const filteredEbooks = useSelector(selectFilteredEbooks);
  ...
  return (
    <div className={wrapper}>
      ...
      {pagesCount && <ul className={paginationWrapper}>
        ...
        {pagesCount > 1 ? <li onClick={(e) => setPage(e.target.innerText * 1)}><span>{page <= 4 ? 2 : (page >= pagesCount - 3) ? pagesCount - 5 : page - 2}</span></li> : null}
        {pagesCount > 2 ? <li onClick={(e) => setPage(e.target.innerText * 1)}><span>{page <= 4 ? 3 : (page >= pagesCount - 3) ? pagesCount - 4 : page - 1}</span></li> : null}
        {pagesCount > 3 ? <li onClick={(e) => setPage(e.target.innerText * 1)}><span>{page <= 4 ? 4 : (page >= pagesCount - 3) ? pagesCount - 3 : page}</span></li> : null}
        {pagesCount > 4 ? <li onClick={(e) => setPage(e.target.innerText * 1)}><span>{page <= 4 ? 5 : (page >= pagesCount - 3) ? pagesCount - 2 : page + 1}</span></li> : null}
        {pagesCount > 5 ? <li onClick={(e) => setPage(e.target.innerText * 1)}><span>{(page <= 4) ? 6 : (page >= pagesCount - 3) ? pagesCount - 1 : page + 2}</span></li> : null}
